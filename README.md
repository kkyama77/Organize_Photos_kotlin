...
- 🖼️ **サムネイル表示とキャッシュ** - 高速表示と効率的なメモリ管理
- 🖱️ **ダブルクリックでデフォルトアプリ起動** - 写真をすぐに開ける
...
### UI/UX
- ⚙️ **右クリックメニュー** - プロパティ編集、デフォルトアプリで開く
...
### Windows (JRE 17 同梱)
- `OrganizePhotos-1.0.2.msi` (約 57 MB)
- Java のインストール不要
- ダブルクリックでインストール完了

### Linux (Debian/Ubuntu)
- `organize-photos_1.0.2-1_amd64.deb` (JRE 17 同梱)
```bash
sudo dpkg -i organize-photos_1.0.2-1_amd64.deb
```

### JAR ファイル（Java 17以上が必要）
- `OrganizePhotos-1.0.2.jar`
```bash
java -jar OrganizePhotos-1.0.2.jar
```
